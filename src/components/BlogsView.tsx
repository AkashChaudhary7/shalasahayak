...
export const BlogsView: React.FC<BlogsViewProps> = ({
  ...
  return (
    <div className="space-y-6 animate-fadeIn font-sans">
      ...
      {/* Tabs Switcher Navigation - Mobile View Dropdown Selector */}
      <div className="block sm:hidden mb-2">
        <div className="relative">
          <select
            value={activeTab}
            onChange={(e) => setActiveTab(e.target.value as any)}
            className="w-full bg-emerald-800 text-white font-black text-xs py-2.5 px-3 pr-8 rounded-xl appearance-none border border-emerald-700 shadow-sm cursor-pointer"
          >
            <option value="guides" className="bg-slate-900 text-white">{isHi ? '1. सहायता ब्लॉग व मार्गदर्शिका' : '1. Help Guides'}</option>
            <option value="hub" className="bg-slate-900 text-white">{isHi ? '2. मास्टर पिलर रिसोर्स हब' : '2. Resource Hub'}</option>
          </select>
          <ChevronDown className="w-4 h-4 text-white absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none" />
        </div>
      </div>

      {/* Desktop View Tabs Switcher */}
      <div className="hidden sm:flex border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-1.5 rounded-2xl shadow-sm gap-1.5">
        <button
