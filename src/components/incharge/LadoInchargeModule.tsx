...
export const LadoInchargeModule: React.FC<LadoInchargeModuleProps> = ({
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-950 p-4 sm:p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-md space-y-6">
      ...
      {/* 2. Primary Submodule Navigation Tabs - Mobile View Dropdown Selector */}
      <div className="block sm:hidden mb-2">
        <label className="block text-[11px] font-bold text-slate-600 dark:text-slate-400 mb-1">
          {lang === 'hi' ? 'अनुभाग का चयन करें (Select Section):' : 'Select Section:'}
        </label>
        <div className="relative">
          <select
            value={activeTab}
            onChange={(e) => setActiveTab(e.target.value as any)}
            className="w-full bg-emerald-700 text-white font-extrabold text-xs py-2.5 px-3 pr-8 rounded-xl appearance-none border border-emerald-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 shadow-sm cursor-pointer"
          >
            <option value="jankari" className="bg-slate-900 text-white">{lang === 'hi' ? '1. योजना जानकारी (Yojana Info)' : '1. Yojana Info'}</option>
            <option value="roster" className="bg-slate-900 text-white">{lang === 'hi' ? '2. पात्र बालिका सूची (Eligible Girls)' : '2. Eligible Girls'}</option>
            <option value="patra" className="bg-slate-900 text-white">{lang === 'hi' ? '3. आवेदन/सत्यापन पत्र (Verification Form)' : '3. Verification Form'}</option>
            <option value="reports" className="bg-slate-900 text-white">{lang === 'hi' ? '4. डीडीओ/पीईईओ रिपोर्ट (DDO/PEEO Report)' : '4. DDO/PEEO Report'}</option>
          </select>
      ...
      <div className="hidden sm:flex flex-wrap gap-1.5 p-1 bg-slate-100 dark:bg-slate-900/60 rounded-2xl border border-slate-200/50 dark:border-slate-800/60">
        <button
