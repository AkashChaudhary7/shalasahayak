...
import { ArrowLeft, Home, Save, Printer, Download, Image as ImageIcon, RotateCcw, Edit, Eye, School, Shield, Table, CreditCard, CheckSquare, ChevronRight, ChevronLeft, Check, Sparkles, Share2, ShieldCheck, ExternalLink } from 'lucide-react';
...
export const KridaShulkMaker: React.FC<{
  ...
  const handleOpenNewPageReport = () => {
    const printArea = previewRef.current || document.getElementById('kridaShulkPreview');
    if (!printArea) return;
...
<head>
  <meta charset="UTF-8">
  <title>${schoolDetails.schoolName || 'विद्यालय'} - क्रीड़ा शुल्क विवरण प्रपत्र</title>
  <style>
    @page { size: A4 portrait; margin: 8mm; }
    body { font-family: 'Mukta', 'Noto Sans Devanagari', 'Segoe UI', sans-serif; color: #000; margin: 0; padding: 20px; background: #fff; line-height: 1.4; }
    .print-btn { background: #059669; color: white; padding: 10px 20px; font-size: 14px; font-weight: bold; border: none; border-radius: 6px; cursor: pointer; margin-bottom: 16px; }
    @media print { .no-print-btn { display: none !important; } body { padding: 0; } }
  </style>
</head>
<body>
  <div style="text-align:center;" class="no-print-btn">
    <button class="print-btn" onclick="window.print()">🖨️ प्रपत्र प्रिंट निकालें (Print A4)</button>
  </div>
  <div>
    ${printArea.innerHTML}
  </div>
        ...
        <div className="space-y-4">
          ...
          <div className="action-bar-top bg-slate-100 dark:bg-slate-800 p-3.5 rounded-2xl flex flex-wrap items-center justify-between gap-2 no-print border border-slate-200 dark:border-slate-700">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button
                onClick={handleOpenNewPageReport}
                className="px-3.5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-black flex items-center space-x-1.5 shadow-sm transition-all cursor-pointer"
                title={lang === 'hi' ? 'रिपोर्ट को नए विंडो/पेज में खोलें' : 'Open report in a new tab/window'}
              >
                <ExternalLink className="w-4 h-4" />
                <span>{lang === 'hi' ? 'नए पेज में खोलें' : 'New Page'}</span>
              </button>
