  ...
  return (
    <nav aria-label="Breadcrumb" className="bg-emerald-950/5 dark:bg-emerald-900/10 border-b border-emerald-900/10 dark:border-emerald-700/20 px-4 py-2.5 mb-4 rounded-xl">
      <ol className="flex items-center flex-wrap gap-1.5 text-xs font-semibold text-emerald-900 dark:text-emerald-300">
        ...
        {currentView === 'useful-tools' && (
          <>
            <ChevronRight className="w-3.5 h-3.5 opacity-60 text-emerald-800" />
            <li className={`font-bold ${!subtab ? 'text-emerald-800 dark:text-emerald-200' : 'text-slate-600 dark:text-slate-400'}`}>
              {lang === 'hi' ? 'उपयोगी टूल्स (Useful Tools)' : 'Useful Tools'}
            </li>
            {subtab && (
              <>
                <ChevronRight className="w-3.5 h-3.5 opacity-60 text-emerald-800" />
                <li className="text-emerald-800 dark:text-emerald-200 font-bold">
                  {subtab === '8thpay' ? (lang === 'hi' ? '8वां वेतन आयोग' : '8th CPC Calc') :
                   subtab === 'salary' ? (lang === 'hi' ? 'राजस्थान वेतन (RSR)' : 'Salary (RSR)') :
                   subtab === 'bonus' ? (lang === 'hi' ? 'दीपावली तदर्थ बोनस' : 'Diwali Bonus') :
                   subtab === 'excel' ? (lang === 'hi' ? 'एक्सेल शीट (कृतिदेव यूनिकोड)' : 'Excel KrutiDev Unicode') :
                   subtab === 'rules' ? (lang === 'hi' ? 'नियम व चरणबद्ध गाइड' : 'Rules & Guide') :
                   subtab}
                </li>
              </>
            )}
