...
export const ThreeDCard: React.FC<ThreeDCardProps> = ({
  ...
  const activeClass = active 
    ...
    : isDragOver
    ? 'border-emerald-500 dark:border-emerald-400 bg-emerald-50/50 dark:bg-emerald-950/40 ring-2 ring-emerald-400/40 scale-105 z-10'
    : isDragging
    ? 'border-dashed border-emerald-500 dark:border-emerald-500 bg-emerald-100/30 dark:bg-emerald-950/30 opacity-40 scale-95'
    : isReorderMode
    ? 'border-amber-300 dark:border-amber-700 bg-amber-50/30 dark:bg-amber-950/20 hover:border-amber-400 animate-pulse-subtle'
  ...
  return (
    <button
      ...
      className={`w-full aspect-square rounded-[16px] sm:rounded-[20px] p-[10px_6px] sm:p-[14px_10px] border shadow-[0_4px_12px_rgba(0,0,0,0.03)] hover:shadow-[0_8px_20px_rgba(0,0,0,0.07)] hover:-translate-y-0.5 active:translate-y-0.5 active:scale-[0.93] active:shadow-inner active:brightness-95 flex flex-col items-center justify-center text-center transition-all duration-150 cursor-grab active:cursor-grabbing group select-none relative animate-card-entrance touch-manipulation threed-icon-card ${activeClass} ${className}`}
      style={{ contentVisibility: 'auto', ...animationStyle }}
    >
      {/* Reorder Mode Drag Grip Indicator */}
      {(isReorderMode || draggable) && (
        <div 
          className="absolute top-1.5 right-1.5 p-0.5 rounded-full bg-slate-100/80 dark:bg-slate-800/80 text-slate-400 group-hover:text-emerald-600 transition-colors pointer-events-none"
          title="Drag handle"
        >
      ...
      {/* Touch/Button Reorder Controls when in Reorder Mode */}
      {isReorderMode && (
        <div className="absolute inset-x-1 bottom-1 flex items-center justify-between z-20" onClick={(e) => e.stopPropagation()}>
          {onMoveLeft && (
            <button
              type="button"
              ...
              title="Move left"
              className={`p-1 rounded-full bg-amber-500 text-slate-950 font-black hover:bg-amber-400 shadow-xs transition-all active:scale-90 ${!canMoveLeft ? 'opacity-30 cursor-not-allowed' : 'cursor-pointer'}`}
            >
          ...
          {onMoveRight && (
            <button
              ...
              title="Move right"
              className={`p-1 rounded-full bg-amber-500 text-slate-950 font-black hover:bg-amber-400 shadow-xs transition-all active:scale-90 ${!canMoveRight ? 'opacity-30 cursor-not-allowed' : 'cursor-pointer'}`}
            >
      ...
      <div 
        aria-hidden="true"
        className={`w-[42px] h-[42px] sm:w-[50px] sm:h-[50px] rounded-[12px] sm:rounded-[15px] ${active ? 'bg-emerald-100 dark:bg-emerald-900/50' : bgTint} flex items-center justify-center group-hover:scale-105 group-active:scale-90 transition-transform duration-150 shadow-[inset_0_1px_2px_rgba(255,255,255,0.8),_0_2px_5px_rgba(0,0,0,0.02)] shrink-0`}
      >
        {typeof icon === 'string' ? (
          <ThreeDIcon name={icon} size={28} className="bg-transparent" />
        ) : (
          icon
        )}
      </div>
      <span className={`text-[0.76rem] sm:text-[0.82rem] font-bold mt-[6px] sm:mt-[8px] leading-[1.15] tracking-tight line-clamp-2 max-w-full px-0.5 ${active ? 'text-emerald-800 dark:text-emerald-200' : 'text-slate-800 dark:text-slate-100'}`}>
        {label}
      </span>
